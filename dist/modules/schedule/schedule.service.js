...
class ScheduleService {
    ...
    constructor() {
        this.scheduleRepository = database_1.AppDataSource.getRepository(schedule_model_1.Schedule);
    ...
    async createSchedule(scheduleData) {
        ...
        await this.checkForOverlappingSchedules(scheduleData.professional_id, scheduleData.day_of_week, scheduleData.start_time, scheduleData.end_time, scheduleData.valid_from, scheduleData.valid_until);
    ...
    async getActiveSchedulesByProfessional(professionalId) {
        const schedules = await this.scheduleRepository.find({
            where: {
    ...
    async getSchedulesWithFilters(filters) {
        ...
        if (filters.valid_date) {
            ...
            queryBuilder.andWhere('(schedule.valid_from IS NULL OR schedule.valid_from <= :validDate) AND ' +
                '(schedule.valid_until IS NULL OR schedule.valid_until >= :validDate)', { validDate });
        }
    ...
    async updateSchedule(scheduleId, updateData) {
        ...
        if (updateData.day_of_week || updateData.start_time || updateData.end_time ||
            ...
            await this.checkForOverlappingSchedules(existingSchedule.professional_id, updateData.day_of_week || existingSchedule.day_of_week, updateData.start_time || existingSchedule.start_time, updateData.end_time || existingSchedule.end_time, updateData.valid_from !== undefined ? updateData.valid_from :
                (existingSchedule.valid_from ? existingSchedule.valid_from.toISOString().split('T')[0] : undefined), updateData.valid_until !== undefined ? updateData.valid_until :
                (existingSchedule.valid_until ? existingSchedule.valid_until.toISOString().split('T')[0] : undefined), scheduleId);
        }
    ...
    async checkForOverlappingSchedules(professionalId, dayOfWeek, startTime, endTime, validFrom, validUntil, excludeScheduleId) {
        const queryBuilder = this.scheduleRepository.createQueryBuilder('schedule')
            ...
            .andWhere('(schedule.start_time < :endTime AND schedule.end_time > :startTime)', { startTime, endTime });
    ...
    mapToWithProfessionalDto(schedule) {
        return {
